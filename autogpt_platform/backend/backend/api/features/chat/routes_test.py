...
# ─── DELETE /sessions/{id} — async sandbox cleanup ─────────────────────


def test_delete_session_schedules_sandbox_cleanup_when_e2b_active(
    mocker: pytest_mock.MockerFixture,
    test_user_id: str,
) -> None:
    mocker.patch(
    ...
    mocker.patch(
        ...
        return_value=type("Cfg", (), {"e2b_active": True, "e2b_api_key": "api-key"})(),
    )
    mock_create_task = mocker.patch(
        "backend.api.features.chat.routes.asyncio.create_task"
    )
    mocker.patch(
        "backend.api.features.chat.routes._cleanup_sandbox",
        new_callable=AsyncMock,
        return_value=None,
    )

    response = client.delete("/sessions/sess-delete")
...
def test_delete_session_skips_sandbox_cleanup_when_e2b_inactive(
    mocker: pytest_mock.MockerFixture,
    test_user_id: str,
) -> None:
    mocker.patch(
    ...
    mocker.patch(
        "backend.api.features.chat.routes.ChatConfig",
        return_value=type("Cfg", (), {"e2b_active": False, "e2b_api_key": None})(),
    )
    mock_create_task = mocker.patch(
        "backend.api.features.chat.routes.asyncio.create_task"
    )
